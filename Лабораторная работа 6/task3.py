...
def to_csv_file(filename, headers, rows, delimiter=',', new_line='\n'):
    with open(filename, 'w') as fout:  # открываем фаил для записи
        fout.write(delimiter.join(headers) + new_line)  # добавляем список заголовков и разделитель строк в фаил
        for i in rows:
            fout.write(delimiter.join(i) + new_line) # добавляем строки данных и разделитель строк в фаил


    return fout


to_csv_file(OUTPUT_FILE, headers_list, data)  # записываем данные в csv фаил
with open(OUTPUT_FILE) as output_fout:  # открываем фаил
    for line in output_fout:
        print(line, end="")  # вывод фаила
